"""Datenanalyse Tool mit Streamlit"""
...
from helper import (
    ...
    BatteryStorage,
    timer,
    create_hourly_bundles,
    create_weekly_bundles,
    create_monthly_bundles,
...
if not project_folders:
    ...
    selected_project = st.sidebar.selectbox(
        "Wähle ein Projekt", project_folders, index=0
    )
...
if not json_files:
    ...
    selected_file2 = st.sidebar.selectbox(
        "Wähle eine zweite Datenreihe", json_files, index=0
    )
    if selected_file2 != "leer":
        json_files.remove(selected_file2)
        files.append(selected_file2)
    selected_file3 = st.sidebar.selectbox(
        "Wähle eine dritte Datenreihe", json_files, index=0
    )
...
with st.sidebar.expander("🌞 PV Simulation"):
    ...
    if enable_pv_simulation and pv_total > 0:
        if st.button(
            "📥 PV-Daten als CSV/JSON speichern",
            help="Speichert die generierte PV-Simulation im Projektordner",
        ):
...
opt_clean_columns = st.sidebar.checkbox(
    "Unnötige Spalten entfernen",
    value=True,
    help="Entfernt alle Spalten außer Datum/Uhrzeit und Wert",
)
opt_etl_steps = st.sidebar.checkbox(
    "Zeige ETL Schritte",
    value=False,
    help="Zeigt die Extract-Transform-Load Verarbeitungsschritte für jede Datenreihe",
)
...
opt_calc = st.sidebar.checkbox(
    "Berechnungen durchführen",
    value=False,
    help="Wenn Last und Erzeugung vorhanden sind, werden Eigenverbrauch, Einspeisung etc. berechnet",
)
...
options = {
    ...
    "battery": {
        ...
        "config_file": selected_battery
        if enable_battery and selected_battery != "Keine"
        else None,
        "config_path": os.path.join(batteries_folder, selected_battery)
        if enable_battery and selected_battery != "Keine"
        else None,
    },
}
...
if options["pv_simulation"]["enabled"] and options["pv_simulation"]["total"] > 0:
    ...
    if os.path.exists(pv_file_path):
        df = pd.read_csv(
            pv_file_path, skiprows=0, decimal=",", sep=";", encoding="utf-8"
        )
        ...
        df["kW"] = (
            df["PV_east_30_norm"] * factor_east
            + df["PV_south_30_norm"] * factor_south
            + df["PV_west_30_norm"] * factor_west
        )
        ...
        st.warning(
            "PV Simulationsdatei nicht gefunden. Bitte sicherstellen, dass 'data/pv_simulation.csv' existiert."
        )
...
if options["battery"]["enabled"] and options["battery"]["config_file"]:
    ...
    # Extrahiere die Batteriekonfiguration aus dem YAML
    if "battery_storage" in battery_config:
        config = battery_config["battery_storage"]
    else:
        config = battery_config

    # Erstelle BatteryStorage-Instanz und wende Konfiguration an
    battery = BatteryStorage()
    battery.apply_config(config)

    # Zeige Batterie-Konfiguration, falls geladen
    if battery is not None:
        battery_info = {k: v for k, v in vars(battery).items() if not k.startswith("_")}
        st.info(
            f"Batteriespeicher geladen:\n{json.dumps(battery_info, indent=2, ensure_ascii=False)}"
        )
...
if len(bundles) > 0:
    ...
    if len(set(row_counts)) > 1:
        st.warning(
            f"Achtung: Die Anzahl der Zeilen unterscheidet sich zwischen den geladenen Datenreihen: {row_counts}"
        )

    if len(set(first_timestamps)) > 1:
        st.warning(
            f"Achtung: Die Zeitstempel der geladenen Datenreihen sind unterschiedlich: {[ts.strftime('%Y-%m-%d %H:%M:%S') for ts in first_timestamps]}"
        )
    ...
    for bundle in bundles:
        ...
        if not df_full["datetime"].equals(bundle.df["datetime"]):
            st.warning(
                f"Achtung: datetime-Spalte von '{bundle.description}' unterscheidet sich vom ersten Bundle. Merge wird durchgeführt."
            )
    ...
    if opt_calc and not options["battery"]["enabled"]:  # type: ignore
        ...
        erzeugung_bundle = next(
            (b for b in bundles if b.description == "Erzeugung"), None
        )
        ...
        if last_bundle is not None and erzeugung_bundle is not None:
            ...
            merged["Eigenverbrauch (kW)"] = merged[["kW_Last", "kW_Erzeugung"]].min(
                axis=1
            )
            # Einspeisung = Erzeugung - Eigenverbrauch
            merged["Einspeisung (kW)"] = (
                merged["kW_Erzeugung"] - merged["Eigenverbrauch (kW)"]
            )
            # Fremdbezug = Last - Eigenverbrauch
            merged["Fremdbezug (kW)"] = (
                merged["kW_Last"] - merged["Eigenverbrauch (kW)"]
            )
            ...
            timer(
                "Zeit nach dem Berechnen von Eigenverbrauch und Darstellung des Dataframes",
                opt_timer,
            )
            ...
            with col1:
                ...
                sum_ev_kwh = merged["Eigenverbrauch (kW)"].sum() * (
                    bundles[0].interval / 60
                )
                ...
                st.write(
                    f"Minimum Eigenverbrauch: {min_ev:,.2f} kW am {min_ev_row['datetime']}"
                )
                st.write(
                    f"Maximum Eigenverbrauch: {max_ev:,.2f} kW am {max_ev_row['datetime']}"
                )

            with col2:
                st.write("#### Einspeisung")
                sum_ein_kwh = merged["Einspeisung (kW)"].sum() * (
                    bundles[0].interval / 60
                )
                ...
                st.write(
                    f"Minimum Einspeisung: {min_ein:,.2f} kW am {min_ein_row['datetime']}"
                )
                st.write(
                    f"Maximum Einspeisung: {max_ein:,.2f} kW am {max_ein_row['datetime']}"
                )

            with col3:
                st.write("#### Fremdbezug")
                sum_fb_kwh = merged["Fremdbezug (kW)"].sum() * (
                    bundles[0].interval / 60
                )
                ...
                st.write(
                    f"Minimum Fremdbezug: {min_fb:,.2f} kW am {min_fb_row['datetime']}"
                )
                st.write(
                    f"Maximum Fremdbezug: {max_fb:,.2f} kW am {max_fb_row['datetime']}"
                )
                ...
                if gesamt_last > 0:
                    ...
                    st.warning(
                        "Gesamtautarkiegrad kann nicht berechnet werden (Summe Last = 0)."
                    )
            ...
            for col in merged.columns:
                if col != "datetime" and col not in df_full.columns:
                    ...
                    df_full = pd.merge(
                        df_full, merged[["datetime", col]], on="datetime", how="left"
                    )
            ...
            st.info(
                "Für die Berechnungen werden sowohl die 'Last'- als auch die 'Erzeugung'-Datenreihen benötigt."
            )
        ...
        if last_bundle is not None and erzeugung_bundle is not None:
            ...
            for index, row in merged.iterrows():
                ...
                if erzeugung > last:
                    ...
                    fremdbezug = (
                        rest_load - discharge_amount
                    )  # Restliche Last nach Entladung
                ...
                df_full.at[index, "Eigenverbrauch (kW)"] = pv_for_load + (
                    discharge_amount / interval_hours
                )
                df_full.at[index, "Einspeisung (kW)"] = einspeisung
                df_full.at[index, "Fremdbezug (kW)"] = fremdbezug
                df_full.at[index, "Speicher Ladung (kW)"] = (
                    charge_amount / interval_hours
                )
                df_full.at[index, "Speicher Entladung (kW)"] = (
                    discharge_amount / interval_hours
                )
            ...
            with col1:
                st.write("#### Eigenverbrauch")
                # Summe als kWh anzeigen (Intervall in Minuten, geteilt durch 60)
                sum_ev_kwh = df_full["Eigenverbrauch (kW)"].sum() * (
                    bundles[0].interval / 60
                )
                st.write(f"Summe Eigenverbrauch: {sum_ev_kwh:,.2f} kWh")
                min_ev = df_full["Eigenverbrauch (kW)"].min()
                max_ev = df_full["Eigenverbrauch (kW)"].max()
                min_ev_row = df_full.loc[df_full["Eigenverbrauch (kW)"].idxmin()]
                max_ev_row = df_full.loc[df_full["Eigenverbrauch (kW)"].idxmax()]
                st.write(
                    f"Minimum Eigenverbrauch: {min_ev:,.2f} kW am {min_ev_row['datetime']}"
                )
                st.write(
            ...
            with col2:
                st.write("#### Einspeisung")
                sum_ein_kwh = df_full["Einspeisung (kW)"].sum() * (
                    bundles[0].interval / 60
                )
                st.write(f"Summe Einspeisung: {sum_ein_kwh:,.2f} kWh")
                min_ein = df_full["Einspeisung (kW)"].min()
                max_ein = df_full["Einspeisung (kW)"].max()
                min_ein_row = df_full.loc[df_full["Einspeisung (kW)"].idxmin()]
                max_ein_row = df_full.loc[df_full["Einspeisung (kW)"].idxmax()]
                st.write(
                    f"Minimum Einspeisung: {min_ein:,.2f} kW am {min_ein_row['datetime']}"
                )
                st.write(
            ...
            with col3:
                st.write("#### Fremdbezug")
                sum_fb_kwh = df_full["Fremdbezug (kW)"].sum() * (
                    bundles[0].interval / 60
                )
                st.write(f"Summe Fremdbezug: {sum_fb_kwh:,.2f} kWh")
                min_fb = df_full["Fremdbezug (kW)"].min()
                max_fb = df_full["Fremdbezug (kW)"].max()
                min_fb_row = df_full.loc[df_full["Fremdbezug (kW)"].idxmin()]
                max_fb_row = df_full.loc[df_full["Fremdbezug (kW)"].idxmax()]
                st.write(
                    f"Minimum Fremdbezug: {min_fb:,.2f} kW am {min_fb_row['datetime']}"
                )
                st.write(
                ...
                # Gesamtautarkiegrad berechnen und anzeigen
                # Autarkiegrad = Eigenverbrauch / Last
                gesamt_eigenverbrauch = df_full["Eigenverbrauch (kW)"].sum()
                gesamt_last = merged["kW_Last"].sum()
                if gesamt_last > 0:
                    autarkiegrad = gesamt_eigenverbrauch / gesamt_last * 100
                    st.success(f"**Gesamtautarkiegrad:** {autarkiegrad:,.2f} %")
            ...
            st.info(
                "Für die Berechnungen mit Batteriespeicher werden sowohl die 'Last'- als auch die 'Erzeugung'-Datenreihen benötigt."
            )
            st.stop()
    ...
    columns_to_plot = [
        ...
        if col != "datetime" and col not in ["kW_Last", "kW_Erzeugung"]
    ]
    ...
    if len(bundles) > 1:
        ...
        reference_index = next(
            ...
            if bundle.description == selected_reference
        )
    else:
        reference_index = 0
    ...
    if (
        bundles_monthly
        and reference_index < len(bundles_monthly)
        and not bundles_monthly[reference_index].df.empty
    ):
        # Finde den Monat mit dem höchsten Wert (kWh)
        strongest_month_row = bundles_monthly[reference_index].df.loc[
            bundles_monthly[reference_index].df["kW"].idxmax()
        ]
        strongest_month = strongest_month_row["datetime"]
        st.info(
            f"Stärkster Monat: {strongest_month.strftime('%B %Y')} mit {strongest_month_row['kW']:,.2f} kW"
        )

        # Filtere df_full auf den Datumsbereich dieses Monats
        mask = df_full["datetime"].dt.to_period("M") == pd.Period(
            strongest_month, freq="M"
        )
        ...
        columns_to_plot = [
            col
            for col in df_strongest_month.columns
            if col != "datetime" and col not in ["kW_Last", "kW_Erzeugung"]
        ]
    ...
    if (
        bundles_weekly
        and reference_index < len(bundles_weekly)
        and not bundles_weekly[reference_index].df.empty
    ):
        # Finde die Woche mit dem höchsten Wert (kW)
        strongest_week_row = bundles_weekly[reference_index].df.loc[
            bundles_weekly[reference_index].df["kW"].idxmax()
        ]
        strongest_week_start = strongest_week_row["datetime"]
        strongest_week_end = strongest_week_start + pd.Timedelta(days=7)
        st.info(
            f"Stärkste Woche: {strongest_week_start.strftime('%d.%m.%Y')} bis {strongest_week_end.strftime('%d.%m.%Y')} mit {strongest_week_row['kW']:,.2f} kW"
        )

        # Filtere df_full auf den Datumsbereich dieser Woche
        mask = (df_full["datetime"] >= strongest_week_start) & (
            df_full["datetime"] < strongest_week_end
        )
        ...
        columns_to_plot = [
            col
            for col in df_strongest_week.columns
            if col != "datetime" and col not in ["kW_Last", "kW_Erzeugung"]
        ]
