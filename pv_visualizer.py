...
def create_daily_overview(df, description):
    ...
    fig = make_subplots(
        ...
        specs=[[{"colspan": 2}, None], [{"type": "bar"}, {"type": "histogram"}]],
    ...
    # Update layout
    fig.update_layout(
        height=800, title_text=f"PV System Analysis: {description}", title_x=0.5
    )
...
def create_heatmap(df, description):
    ...
    pivot_data = df.pivot_table(
        values=kw_col, index="hour", columns="day_of_year", aggfunc="mean"
    )
...
def create_statistics_table(df, description, interval_minutes=15):
    ...
    stats = [
        {"Metric": "Annual Energy (kWh)", "Value": f"{annual_energy:,.0f}"},
        {"Metric": "Daily Average (kWh)", "Value": f"{annual_energy / 365:.1f}"},
        {"Metric": "Max Power (kW)", "Value": f"{max_power:.2f}"},
        {"Metric": "Avg Power (kW)", "Value": f"{avg_power:.2f}"},
        {
...
def main():
    ...
    st.set_page_config(page_title="PV Data Visualization", page_icon="☀️", layout="wide")
    ...
    st.sidebar.metric(
        "Date Range",
        f"{df_filtered['datetime'].min().date()} to {df_filtered['datetime'].max().date()}",
    )
    ...
    with col2:
        ...
        if len(df_filtered) > 0 and kw_col:
            ...
            st.metric(
                "Capacity Factor",
                f"{(avg_power / peak_power * 100):.1f}%" if peak_power > 0 else "0%",
            )
